...
namespace MiBand5WatchFaces.Forms
{
    public partial class BatteryForm : Form
    {
        ...
        public BatteryForm(WatchFaceLibrary watch, DefaultDictionary<int, Image> Images, StateWatchface state)
        {
            ...
            oldBattery = JsonConvert.SerializeObject(this.watch.Battery, Formatting.None, new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore });
        ...
        {
            if (Save == false && oldBattery != JsonConvert.SerializeObject(this.watch.Battery, Formatting.None, new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore }) && MessageBox.Show(res.GetString("ExitMessage"), res.GetString("ExitMessageCaption"), MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
            {
                Save = true;
